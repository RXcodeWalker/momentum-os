  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap gap-1 text-[10px] pb-2 border-b border-border">
        {(Object.entries(trace.layerSummary) as [AdaptationLayer, number][]).map(
          ([layer, count]) => (
            <span
              key={layer}
              className={`px-1.5 py-0.5 rounded text-[10px] font-mono ${LAYER_COLORS[layer]}`}
            >
              {layer}: {count}
            </span>
          ),
          ...
          <tbody>
            {trace.entries.map((entry, i) => (
              <tr key={i} className="border-t border-border/30">
                <td
                  className="py-0.5 pr-2 text-foreground truncate max-w-[120px]"
                  title={entry.field}
                >
