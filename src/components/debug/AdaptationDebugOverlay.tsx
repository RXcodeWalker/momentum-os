...
function ScalarRow({ label, value, max = 100 }: { label: string; value: number; max?: number }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      ...
      <span className="text-[9px] font-mono text-foreground w-8 text-right">{typeof value === 'number' ? value.toFixed(2) : value}</span>
    </div>
  )
}
      ...
      <SheetContent side="right" className="w-80 overflow-y-auto p-4 space-y-3 text-[11px]">
        ...
        <div className="flex gap-1 flex-wrap">
          {(['state', 'output', 'trace', 'runner'] as const).map(t => (
            <button key={t} className={tabClass(t)} onClick={() => setActiveTab(t)}>{t}</button>
          ))}
        </div>
        ...
        {activeTab === 'state' && pipeline && (
          <div className="space-y-2">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs font-mono font-medium text-foreground">{pipeline.stateInterpretation.currentMode}</span>
              <span className="text-[9px] text-muted-foreground ml-2">Trajectory</span>
              <span className="text-xs font-mono text-foreground">{pipeline.stateInterpretation.currentTrajectory}</span>
            </div>
            <div className="flex flex-wrap gap-1">
              {badges.map(b => (
                <span key={b.label} className={`px-1.5 py-0.5 rounded border text-[9px] font-mono ${riskColor(b.value)}`}>
                  {b.label}: {b.value}
                </span>
              ))}
            </div>
            {output && (
              <div className="space-y-0.5 pt-1">
                <p className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">Adaptation Intensity</p>
                <div className="flex items-center gap-2">
                  <span className="text-2xl font-mono font-bold text-foreground">{output.adaptationIntensity.toFixed(0)}</span>
            ...
            {pipeline.signalSnapshot.activeSignals.length > 0 && (
              <div>
                <p className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">Active Signals</p>
                  ...
                  {pipeline.signalSnapshot.activeSignals.map(sig => {
                    ...
                    return (
                      <div key={sig} className="flex items-center gap-1">
                        <span className="text-[9px] font-mono text-foreground w-32 truncate">{sig.replace(/_/g, ' ')}</span>
                        <div className="flex-1 h-1 bg-secondary rounded-full overflow-hidden">
                          <div className="h-full bg-purple-400 rounded-full" style={{ width: `${str}%` }} />
                        </div>
                        <span className="text-[9px] font-mono text-muted-foreground w-5 text-right">{str}</span>
                      </div>
                    )
                  })}
                </div>
        ...
        {activeTab === 'output' && output && (
          <div className="space-y-3">
            <div>
              <p className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">Environmental</p>
              <div className="space-y-0.5">
                ...
                <ScalarRow label="hierarchySharpness" value={output.environmental.hierarchySharpness} />
                <ScalarRow label="contrastStrength" value={output.environmental.contrastStrength} />
                <ScalarRow label="visibleComplexity" value={output.environmental.visibleComplexity} />
                <ScalarRow label="dashboardCompression" value={output.environmental.dashboardCompressionLevel} />
                <div className="flex items-center gap-1.5">
                  <span className="text-[9px] text-muted-foreground w-28">deepWorkProtection</span>
                  <span className={`text-[9px] font-mono px-1 rounded ${output.environmental.deepWorkProtectionEnabled ? 'bg-success/20 text-success' : 'bg-secondary text-muted-foreground'}`}>
            ...
            <div>
              <p className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">Execution</p>
              <div className="space-y-0.5">
                <ScalarRow label="visibleTaskLimit" value={output.execution.visibleTaskLimit} max={10} />
                <ScalarRow label="challengeLevel" value={output.execution.recommendedChallengeLevel} />
                <ScalarRow label="compressionRatio" value={output.execution.workloadCompressionRatio * 100} />
                <ScalarRow label="deepWorkExpect" value={output.execution.deepWorkExpectation} />
                <ScalarRow label="recoveryWeight" value={output.execution.recoveryWeighting * 100} />
                <ScalarRow label="advancementWeight" value={output.execution.advancementWeighting * 100} />
                <ScalarRow label="focusProtection" value={output.execution.focusProtectionStrength} />
                <div className="flex items-center gap-1.5 pt-0.5">
                  <span className="text-[9px] text-muted-foreground w-28">pacingRecommendation</span>
                  <span className="text-[9px] font-mono bg-blue-500/20 text-blue-400 px-1 rounded">{output.execution.pacingRecommendation}</span>
                </div>
              </div>
            </div>
            <div>
              <p className="text-[9px] uppercase tracking-wider text-muted-foreground mb-1">Guidance</p>
              <div className="space-y-0.5">
                ...
                <ScalarRow label="strategicGuidance" value={output.guidance.strategicGuidanceWeight} />
                <ScalarRow label="emotionalPressure" value={output.guidance.emotionalPressureLevel} />
                ...
                <div className="flex items-center gap-1.5 pt-0.5">
                  ...
                  <span className="text-[9px] font-mono bg-purple-500/20 text-purple-400 px-1 rounded">{output.guidance.messagingTone}</span>
                </div>
              </div>
            </div>
          </div>
