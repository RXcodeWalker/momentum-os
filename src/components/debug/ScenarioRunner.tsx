...
import {
  SCENARIO_RECOVERY_CONTRACTING, SCENARIO_RECOVERY_FRAGILE,
  SCENARIO_RECOVERY_STABLE, SCENARIO_RECOVERY_EXPANDING,
  SCENARIO_STABILIZING_CONTRACTING, SCENARIO_STABILIZING_FRAGILE,
  SCENARIO_STABILIZING_STABLE, SCENARIO_STABILIZING_EXPANDING,
  SCENARIO_FOCUSED_CONTRACTING, SCENARIO_FOCUSED_FRAGILE,
  SCENARIO_FOCUSED_STABLE, SCENARIO_FOCUSED_EXPANDING,
  SCENARIO_EXPANDING_CONTRACTING, SCENARIO_EXPANDING_FRAGILE,
  SCENARIO_EXPANDING_STABLE, SCENARIO_EXPANDING_EXPANDING,
...
const SIGNALS: BehavioralSignal[] = [
  'DEEP_WORK_DEGRADATION', 'RISING_FRAGMENTATION', 'VOLATILITY_ACCELERATION',
  'AVOIDANCE_CLUSTERING', 'OVERCOMMITMENT_EXPANSION', 'PLANNING_ESCAPE',
  'MEANINGFULNESS_DEFERRAL', 'DECLINING_EXECUTION_QUALITY', 'PACING_INSTABILITY',
]
...
export function ScenarioRunner({ onOutput }: Props) {
  ...
  function loadPreset(label: string) {
    ...
    setSignalStrengths(p.input.signalSnapshot.signalStrengths as Partial<Record<BehavioralSignal, number>>)
    setPreset(label)
  }
  ...
  function buildInput(): AdaptationEngineInput {
    ...
    return {
      stateInterpretation: {
        recoveryDebt: 30, cognitiveStrain: 28, executionStability: 72,
        emotionalFriction: 25, momentumIntegrity: 68, resilienceCapacity: 70,
        overwhelmLevel: 22, fragmentationLevel: 20, recoveryCapacity: 70,
        meaningfulEngagement: 72, deepWorkContinuity: 75, behavioralVolatility: 18,
        currentMode: mode, currentTrajectory: trajectory,
        overloadRisk: overload, burnoutRisk: burnout,
        avoidanceRisk: avoidance, collapseRisk: collapse,
        adaptationReadiness: 72, expansionReadiness: 68,
        consistencyTrend: 'STABLE', recoveryTrend: 'STABLE', engagementTrend: 'STABLE',
        confidence: { score: 75, band: 'HIGH', evidenceCoverage: 0.85, signalConsistency: 0.8, uncertaintyFactors: [] },
      ...
      interventionEvaluation: {
        interventions: [], evaluationNotes: [],
        restraintApplied: false, candidatesFound: 0, engineVersion: 'v1',
      },
    }
  }
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-col gap-0.5">
        <label className="text-[9px] uppercase tracking-wider text-muted-foreground">Load Preset</label>
        <select
          value={preset}
      ...
      <div className="space-y-1">
        <p className="text-[9px] uppercase tracking-wider text-muted-foreground">Signal Strengths</p>
        {SIGNALS.map(signal => (
          <div key={signal} className="flex items-center gap-1">
            <span className="text-[9px] font-mono text-muted-foreground w-36 truncate">{signal.replace(/_/g, ' ')}</span>
            <input
              type="range"
              min={0}
              max={100}
              value={signalStrengths[signal] ?? 0}
              onChange={e => setSignalStrengths(prev => ({ ...prev, [signal]: Number(e.target.value) }))}
              className="flex-1 h-1 accent-accent"
            />
            <span className="text-[9px] font-mono text-foreground w-5 text-right">{signalStrengths[signal] ?? 0}</span>
          </div>
        ))}
      </div>
