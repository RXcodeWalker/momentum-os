...
import { useApp, useExecutionScore, useScoreAttribution, useStreakContext } from "@/lib/store";
...
export function useEveningReflection(): EveningReflectionView {
  ...
  return useMemo(() => {
    ...
    const stateHeadline = lastPipelineResult?.stateExplanation?.primary?.observation ?? null;
    ...
    const workloadMessage = lastPipelineResult?.stateExplanation?.primary?.observation ?? null;
