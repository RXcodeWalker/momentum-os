...
export function useBehavioralExecution(): BehavioralExecutionView {
  ...
  return useMemo((): BehavioralExecutionView => {
    ...
    const { visibleTaskIds, hiddenCount, compressionReason } = applyVisibilityRules(orderedIds, execution)
    ...
    // Deep work nudge
    const needsNudge =
      execution.deepWorkExpectation > 60 &&
      incompleteTasks.length > 0 &&
      !focusEnvironment?.active
    ...
    return {
      ...
      compressionReason,
      stretchOpportunity:
        stretchTaskId && stretchPrompt
          ? { taskId: stretchTaskId, prompt: stretchPrompt }
          : null,
