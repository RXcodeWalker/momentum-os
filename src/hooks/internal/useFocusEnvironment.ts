...
export function useFocusEnvironment(): FocusEnvironmentView {
  ...
  return useMemo((): FocusEnvironmentView => {
    ...
    const hasOverloadBlock = interventions.some((i) => i.type === "OVERLOAD" && i.level >= 1);
    const hasRestartBlock = interventions.some((i) => i.type === "RESTART_ASSISTANCE");
    ...
    // --- entryAutoSuggested ---
    const hasDwpTrigger =
      behavioral.shell.focusMode || interventions.some((i) => i.type === "DEEP_WORK_PROTECTION");
    const hasFragmentationTrigger = interventions.some((i) => i.type === "FRAGMENTATION_REDUCTION");
    ...
    const entryAutoSuggested =
      entryAllowed && !active && (hasDwpTrigger || hasFragmentationTrigger) && !dismissedRecently;
    ...
    if (active) {
      ...
      hiddenCount = mode === "RECOVERY" ? 0 : Math.max(0, incompleteTasks.length - visibleCount);
    } else {
      focusSecondaryTask = secondaryTask;
    }
    ...
    const heldInterventions = active ? interventions.filter((i) => i.level === 2) : [];
