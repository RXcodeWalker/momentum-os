...
const INTERVENTION_BASE_COPY: Record<string, string> = {
  BURNOUT_PREVENTION:
    "Your load pattern suggests recovery time is needed before output drops further.",
  ...
  AVOIDANCE_INTERRUPTION:
    "A task has been pending for multiple sessions. A small start breaks the pattern.",
  FRAGMENTATION_REDUCTION:
    "Frequent context-switching is reducing effective output. Consolidate your focus window.",
  DEEP_WORK_PROTECTION: "Your deep work block is at risk. Protect the next 90 minutes.",
  RESTART_ASSISTANCE:
    "Returning after a gap — the most effective re-entry is a single small win today.",
};
