...
export function generateCheckInPrompts(
  ...
  dateSeed: string,
  reasoning: string[],
): Partial<Record<'checkin-open-question' | 'checkin-pattern-question' | 'checkin-forward-question', GuidanceMessage>> {
  const provenance = buildProvenance(ctx)
  const result: Partial<Record<'checkin-open-question' | 'checkin-pattern-question' | 'checkin-forward-question', GuidanceMessage>> = {}
