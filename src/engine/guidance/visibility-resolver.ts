...
import type { PromptVisibilityDirective, PromptSurface, InterventionVisibilityRule } from '@/core/contracts/guidance/visibility'
...
export function resolvePromptVisibility(
  ...
  if (interventionFrequency <= FREQUENCY_THRESHOLDS.LOW_MAX) {
    reasoning.push(`Prompt visibility: LOW (frequency=${interventionFrequency}) — morning only, max 1/session.`)
    return {
  ...
  if (interventionFrequency <= FREQUENCY_THRESHOLDS.MODERATE_MAX) {
    reasoning.push(`Prompt visibility: MODERATE (frequency=${interventionFrequency}) — morning + evening, max 2/session.`)
    return {
      allowMorningPrompts: true,
      allowMiddayPrompts: false,
  ...
  if (interventionFrequency <= FREQUENCY_THRESHOLDS.HIGH_MAX) {
    reasoning.push(`Prompt visibility: HIGH (frequency=${interventionFrequency}) — all surfaces except in-session, max 3/session.`)
    return {
      allowMorningPrompts: true,
  ...
  reasoning.push(`Prompt visibility: FULL (frequency=${interventionFrequency}) — all surfaces, unlimited.`)
  return {
    allowMorningPrompts: true,
    allowMiddayPrompts: true,
    allowEveningPrompts: true,
