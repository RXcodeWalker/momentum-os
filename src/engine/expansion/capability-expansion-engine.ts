...
function computeReadinessAndSignals(
  ...
  let recoveryScore = 0
  if (successRate >= 0.80) recoveryScore = 10
  else if (successRate < 0.50) recoveryScore = -10
  ...
  if (consistency > 0.80) avoidanceScore += 10
  else if (consistency < 0.50) avoidanceScore -= 5
...
function buildRationale(
  ...
  switch (directive) {
    ...
    case 'hold':
      ...
      break
    case 'reduce':
      parts.push(`Challenge reduced to relieve pressure (pace factor ×${paceModifier.toFixed(2)}).`)
      break
  }
...
export function computeExpansionDecision(
  ...
  const { momentumModel, stateDynamics, dynamicsProfile, recoveryMode, streakAtRisk,
          recoveryDebtAccumulating, avoidancePressure, checkInsCount } = input
  ...
  const rationale = buildRationale(directive, readinessScore, blocking, triggeredBlockers, paceModifier)
