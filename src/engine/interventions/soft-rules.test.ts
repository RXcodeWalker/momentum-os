...
function makeCandidate(type: InterventionCandidate['type'] = 'OVERLOAD'): InterventionCandidate {
  return { type, matchedSignals: ['RISING_FRAGMENTATION'], minSignalDuration: 3, evidenceScore: 75 }
}

function makeAssessment(type: InterventionCandidate['type'], maxLevel: 0 | 1 | 2 | 3 = 2): EligibilityAssessment {
  return {
    candidateType: type,
...
function makeSnapshot(confidence: 'LOW' | 'MEDIUM' | 'HIGH' = 'HIGH'): SignalSnapshot {
  return makeSignalSnapshot() // task fixture returns empty signals with confidence
    ? { capturedAt: new Date().toISOString(), activeSignals: [], signalStrengths: {}, signalDurations: {}, confidence }
    : { capturedAt: new Date().toISOString(), activeSignals: [], signalStrengths: {}, signalDurations: {}, confidence }
}
...
describe('soft rule: sequencingSaturated', () => {
  it('downgrades to level 0 when ≥50% of tasks are suppressed', () => {
    const saturated = makeSequencing({ suppressedTaskIds: ['t1', 't2'], compressedTaskIds: ['t3'] })
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD')], state, makeSnapshot(), saturated, [], NOW)
  ...
  it('does not downgrade when suppression ratio < 50%', () => {
    const light = makeSequencing({ suppressedTaskIds: ['t1'], compressedTaskIds: ['t2', 't3', 't4'] })
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD')], state, makeSnapshot(), light, [], NOW)
  ...
  it('does not downgrade when no tasks are involved', () => {
    ...
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD')], state, makeSnapshot(), empty, [], NOW)
    expect(verdicts[0].downgradeLevel).toBeUndefined()
  })
})
...
describe('soft rule: lowConfidenceDowngrade', () => {
  it('downgrades to level 1 when snapshot confidence is LOW and maxLevel > 1', () => {
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 2)], state, makeSnapshot('LOW'), makeSequencing(), [], NOW)
    expect(verdicts[0].downgradeLevel).toBe(1)
  })

  it('does not downgrade when confidence is MEDIUM', () => {
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 2)], state, makeSnapshot('MEDIUM'), makeSequencing(), [], NOW)
    expect(verdicts[0].downgradeLevel).toBeUndefined()
  })

  it('does not downgrade when maxLevel is already ≤1', () => {
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 1)], state, makeSnapshot('LOW'), makeSequencing(), [], NOW)
    expect(verdicts[0].downgradeLevel).toBeUndefined()
  })
})
...
describe('soft rule: interventionFatigue', () => {
  it('downgrades to level 1 when a level-1+ intervention fired in last 24h', () => {
    ...
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 2)], state, makeSnapshot(), makeSequencing(), recent, NOW)
    expect(verdicts[0].downgradeLevel).toBe(1)
  })

  it('does not downgrade when recent intervention fired > 24h ago', () => {
    const old = [makeAuditRecord(1, 25)]
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 2)], state, makeSnapshot(), makeSequencing(), old, NOW)
    expect(verdicts[0].downgradeLevel).toBeUndefined()
  })

  it('does not downgrade when maxLevel is already ≤1', () => {
    const recent = [makeAuditRecord(1, 12)]
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 1)], state, makeSnapshot(), makeSequencing(), recent, NOW)
    expect(verdicts[0].downgradeLevel).toBeUndefined()
  })
})
...
describe('soft rule: priority order', () => {
  it('saturation takes precedence over fatigue when both apply', () => {
    const saturated = makeSequencing({ suppressedTaskIds: ['t1', 't2'], compressedTaskIds: ['t3'] })
    const recent = [makeAuditRecord(1, 12)]
    const verdicts = applySoftRules([makeCandidate()], [makeAssessment('OVERLOAD', 2)], state, makeSnapshot(), saturated, recent, NOW)
    expect(verdicts[0].downgradeLevel).toBe(0)
    expect(verdicts[0].rule).toBe('SEQUENCING_SATURATED')
  })
})
