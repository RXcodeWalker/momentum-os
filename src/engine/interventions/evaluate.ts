...
} from "@/core/contracts/interventions/evaluation";
...
export function evaluateInterventions(
  ...
  const { recentInterventions, activeReentryProtocol, intelligenceAdvisories } = context;
  ...
  // Stage 3 — Cooldown (optional intelligence multipliers applied here)
  const cooldownVerdicts = evaluateCooldown(
    eligibleCandidates,
    recentInterventions,
    nowMs,
    intelligenceAdvisories?.cooldown,
  );
  ...
  if (postCooldown.length === 0) {
    return emptyResult(buildEvaluationNotes([], cooldownVerdicts, []), true, candidates.length);
  }

  // Stage 4 — Suppression (optional intelligence DEMOTE advisories applied here)
  const suppressionResults = evaluateSuppression(
    ...
    nowMs,
    intelligenceAdvisories?.suppression,
  );
  ...
  const restraintApplied =
    cooldownBlocked || suppressionApplied || level < winnerAssessment.maxAllowedLevel;
