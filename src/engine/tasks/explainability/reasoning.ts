...
function primarySelectionFactor(
  task: Task,
  ...
  switch (mode) {
    case 'RECOVERY':
      return {
        ...
        observation:
          'This task may align with current recovery capacity based on recent patterns.',
        influence: 'supports',
      }
    case 'STABILIZING':
      return {
...
function buildSuppressionFactors(
  ...
  return suppressedIds.map(id => {
    ...
    return {
      ...
      observation: compat?.band === 'HARMFUL'
...
export function buildReasoningTrace(
  ...
  if (factors.length === 0) {
    factors.push({
      ...
      observation:
        'Available task evidence is limited; recommendations carry lower certainty.',
      influence: 'neutral',
    })
  }
