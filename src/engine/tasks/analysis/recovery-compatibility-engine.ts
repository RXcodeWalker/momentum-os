  ...
  if (dims) {
    ...
    debtSensitivity =
      (d.exhaustionAccumulation * 0.35 +
        (100 - d.sleepQuality) * 0.3 +
        d.sustainedIntensity * 0.25 +
        (100 - d.recoveryBehaviorQuality) * 0.1) /
      100;
  } else {
...
export function computeEmotionalFrictionLoad(
  ...
  if (dims) {
    ...
    frictionLevel =
      (d.initiationResistance * 0.3 +
        d.avoidancePressure * 0.3 +
        d.perfectionismPressure * 0.15 +
        d.overwhelmWeight * 0.15 +
        d.uncertaintyResistance * 0.1) /
      100;
  } else {
  ...
  return clampScalar(
    frictionLevel * 50 + taskResistance * 50 + frictionLevel * taskResistance * 50,
  );
}
...
export function detectFlags(
  ...
  const grossLoad =
    components.recoveryDebtLoad * 0.3 +
