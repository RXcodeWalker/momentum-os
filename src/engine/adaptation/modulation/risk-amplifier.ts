...
export function applyRiskAmplification(
  ...
  for (const entry of RISK_GATES) {
    ...
    for (const gate of entry.gates) {
      ...
      if (prev !== next) {
        ...
        recorder.record(`${domainFor(field as string)}.${field}`, prev, next, "risk", gate.reason);
        anyFired = true;
      }
    }
  }
  ...
  if (anyFired) {
    draft.reasoning.push(
      `Risk gates applied (burnout=${ctx.burnoutRisk}, overload=${ctx.overloadRisk}, avoidance=${ctx.avoidanceRisk}, collapse=${ctx.collapseRisk})`,
    );
  }
}
