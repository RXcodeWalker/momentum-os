...
import { TRAJECTORY_ENV_DELTAS, TRAJECTORY_EXEC_DELTAS, TRAJECTORY_GUIDANCE_DELTAS } from '../config'
...
export function applyTrajectoryDelta(
  ...
  function applyField<K extends keyof AdaptationDraft>(
    field: K,
    delta: number,
    domain: string,
  ) {
  ...
  for (const [f, delta] of Object.entries(guidanceDelta) as [keyof typeof guidanceDelta, number][]) {
    applyField(f as keyof AdaptationDraft, delta, 'guidance')
  }

  if (Object.keys(envDelta).length + Object.keys(execDelta).length + Object.keys(guidanceDelta).length > 0) {
