...
describe("Invariant tests: 50 random inputs", () => {
  ...
  it.each(SEEDS)(
    "seed %i: weight normalization |recoveryWeighting + advancementWeighting - 1.0| ≤ 0.001",
    (seed) => {
      const { output } = RANDOM_OUTPUTS[seed];
  ...
  it.each(SEEDS)(
    "seed %i: DEEP_WORK_DEGRADATION strength > 0 → deepWorkProtectionEnabled = true",
    (seed) => {
      const { input, output } = RANDOM_OUTPUTS[seed];
