...
describe('applyExpansionModulation', () => {
  it('no-op when expansionDirective is absent', () => {
    ...
    applyTrajectoryDelta(draft, makeCtx({ trajectory: 'EXPANDING', mode: 'FOCUSED' }), makeNoopRecorder())
  ...
  it('no-op when expansionPaceModifier is absent', () => {
    const draft = applyModeBaseline('FOCUSED', makeNoopRecorder())
    const before = draft.recommendedChallengeLevel
    applyExpansionModulation(draft, makeCtx({ expansionDirective: 'increase' }), makeNoopRecorder())
    expect(draft.recommendedChallengeLevel).toBe(before)
  })
  ...
  describe('directive: gradual_increase', () => {
    ...
    it('gradual_increase gives less delta than increase at same pace', () => {
      ...
      applyExpansionModulation(draftA, makeCtx({ expansionDirective: 'increase', expansionPaceModifier: 0.8 }), makeNoopRecorder())
      applyExpansionModulation(draftB, makeCtx({ expansionDirective: 'gradual_increase', expansionPaceModifier: 0.8 }), makeNoopRecorder())
  ...
  describe('directive: reduce', () => {
    it('subtracts at least 30% of MAX_CONTRACTION even at pace=1.0', () => {
      ...
      expect(delta).toBeGreaterThanOrEqual(MAX_CONTRACTION * 0.30 - 0.001)
    })
...
function makeStateDynamics(overrides: Partial<StateDynamics> = {}): StateDynamics {
  return {
    ...
    recentTransitions: [],
    stability: { currentModeDays: 14, rating: 'stable', longestStablePeriodDays: 21, longestStablePeriodType: 'FOCUSED' },
    volatility: { score: 30, trend: 'stable', scoreStdDev14d: 5, interpretation: 'stable' },
    oscillation: { isOscillating: false, frequencyPer28Days: 0, dominantCyclePair: null, cycleCount: 0 },
    recoveryCycles: { count: 2, avgDurationDays: 3, lastDurationDays: 2, longestDurationDays: 5, successRate: 0.90 },
    trajectoryShifts: [],
    evidenceDays: 28,
    confidence: 'high',
    ...overrides,
...
function makeStateDynamicsProfile(overrides: Partial<StateDynamicsProfile> = {}): StateDynamicsProfile {
  return {
    transitionMatrix: { paths: [], commonPaths: [], rarePaths: [], totalTransitions: 0 },
    stateStatistics: {},
    recoveryPathwayAnalysis: { pathways: [], failedRecoveries: 0, avgDaysToFocusedFromRecovery: null, mostCommonExitState: null, mostSuccessfulPathway: null },
    instabilityHotspots: [],
    oscillation: { isOscillating: false, frequencyPer28Days: 0, dominantCyclePair: null, cycleCount: 0 },
...
describe('computeExpansionDecision — scenario matrix', () => {
  it('Scenario 1: Oscillating + volatile → hold (OSCILLATION_GATE + VOLATILITY_GATE block)', () => {
    const input = makeInput({
      stateDynamics: makeStateDynamics({
        volatility: { score: 80, trend: 'increasing', scoreStdDev14d: 15, interpretation: 'volatile' },
        oscillation: { isOscillating: true, frequencyPer28Days: 3, dominantCyclePair: null, cycleCount: 3 },
      }),
    })
    const decision = computeExpansionDecision(input, 'EXPANDING', 'EXPANDING')
    expect(decision.directive).toBe('hold')
    expect(decision.safetyConstraints.find((c) => c.id === 'VOLATILITY_GATE')?.triggered).toBe(true)
    expect(decision.safetyConstraints.find((c) => c.id === 'OSCILLATION_GATE')?.triggered).toBe(true)
  })
  ...
  it('Scenario 3: Recovery mode → reduce', () => {
    ...
    expect(decision.safetyConstraints.find((c) => c.id === 'RECOVERY_MODE_GATE')?.triggered).toBe(true)
  })
  ...
  it('paceModifier is always in [0, 1]', () => {
    const inputs: ExpansionEngineInput[] = [
      ...
      makeInput({ momentumModel: makeMomentumModel({ classification: 'contracting', velocity: 'stalled', quality: { fragilityScore: 80, sustainabilityScore: 20, isStructurallySound: false } }) }),
    ]
