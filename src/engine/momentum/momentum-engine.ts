...
export function computeMomentumModel(input: MomentumModelInput): MomentumModel {
  ...
  const invertedRecoverySuccessRate =
    recoverySuccessRate !== null ? (1 - recoverySuccessRate) * 100 : 0;
  ...
  const totalWeight = avoidanceActive ? 1.1 : 1.0;

  const fragilityScore = clamp(
    (volatilityScore * 0.35 +
      (isOscillating ? 100 : 0) * 0.2 +
      invertedRecoverySuccessRate * 0.25 +
      (streakAtRisk ? 100 : 0) * 0.1 +
      (recoveryDebtAccumulating ? 100 : 0) * 0.1 +
      avoidanceFragilityContribution * 0.1) /
      totalWeight,
    0,
    100,
  );
  ...
  const sustainabilityScore = clamp(
    consistency * 0.4 +
      (recoverySuccessRate !== null ? recoverySuccessRate : 0.5) * 100 * 0.3 +
  ...
  const direction = trajectoryToDirection(trajectoryFromPipeline, momentum.trend);
  ...
  if (direction === "DOWN") {
    ...
    if (fragilityScore < FRAGILITY_SOUND_THRESHOLD) {
      ...
      classification =
        velocity === "accelerating" || velocity === "steady" ? "expanding" : "fragile";
    }
  } else {
    // direction === FLAT
    if (fragilityScore < FRAGILITY_SOUND_THRESHOLD) {
  ...
  const signals: MomentumSignal[] = [
    ...
    },
    {
      ...
      contribution:
        recoverySuccessRate !== null && recoverySuccessRate >= 0.5 ? "stability" : "fragility",
      weight: 0.25,
