...
export function analyzeTrajectory(evidence: SessionEvidence[]): UserTrajectory {
  ...
  const recentMean = smoothed.slice(-3).reduce((s, v) => s + v, 0) / Math.min(3, smoothed.length);
  ...
  if (trend === "DECLINING") {
    ...
    const firstSegMean =
      smoothed.slice(0, Math.max(1, Math.floor(smoothed.length / 3))).reduce((s, v) => s + v, 0) /
      Math.max(1, Math.floor(smoothed.length / 3));
