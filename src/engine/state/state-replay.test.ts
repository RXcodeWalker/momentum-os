...
function trajectoryAt(evidence: ReturnType<typeof makeEvidence>[], dayIndex: number): string {
  ...
  return evaluate({ evidence: slice, signalSnapshots: [emptySnapshot(dayIndex)] }).state.currentTrajectory
}
...
describe('Replay R-1: Burnout Spiral', () => {
  const strong = buildEvidence(14, {
    sleepQuality: 80, physicalEnergy: 78, mentalClarity: 78,
    overwhelm: 22, emotionalResistance: 20, stressPressure: 22,
    meaningfulAdvancementQuality: 78, deepWorkContinuity: 76,
    executionIntegrity: 78, fragmentationLevel: 20, avoidancePressure: 20,
  })
...
describe('Replay R-2: Avoidance Cluster', () => {
  const baseline = buildEvidence(10, {
    sleepQuality: 72, physicalEnergy: 70, mentalClarity: 70,
    overwhelm: 28, avoidancePressure: 25, emotionalResistance: 25,
    meaningfulAdvancementQuality: 70, executionIntegrity: 70,
  })

  const avoidance = Array.from({ length: 5 }, (_, i) =>
    makeEvidence(10 + i, {
      // Sleep deliberately stable — pure friction mode
      sleepQuality: 72, physicalEnergy: 68, mentalClarity: 66,
      overwhelm: 28 + (i + 1) * 8,
...
describe('Replay R-3: Recovery Rebuild', () => {
  const crisis = buildEvidence(3, {
    sleepQuality: 22, physicalEnergy: 24, mentalClarity: 28,
    overwhelm: 88, emotionalResistance: 78, stressPressure: 70,
    meaningfulAdvancementQuality: 32, executionIntegrity: 28,
    fragmentationLevel: 72, avoidancePressure: 68,
  })
  ...
  it('recoveryDebt decreases monotonically through the rebuild', () => {
    const debts = rebuild.map((_, i) => {
      ...
      return evaluate({ evidence: slice, signalSnapshots: [emptySnapshot(3 + i)] }).state.recoveryDebt
    })
...
describe('Replay R-4: Stable Expansion', () => {
  const sustained = buildEvidence(14, {
    sleepQuality: 82, physicalEnergy: 80, mentalClarity: 78,
    overwhelm: 20, emotionalResistance: 18, stressPressure: 20,
    meaningfulAdvancementQuality: 80, deepWorkContinuity: 78,
    executionIntegrity: 78, fragmentationLevel: 18, avoidancePressure: 18, pacingQuality: 78,
  })
...
describe('Replay R-5: Cold Restart', () => {
  ...
  it('confidence rises as evidence accumulates', () => {
    ...
    const conf4 = evaluate({ evidence: days4, signalSnapshots: [emptySnapshot(3)] }).state.confidence.score

    expect(conf4).toBeGreaterThan(conf1)
  })

  it('even a very bad single day does not trigger RECOVERY (sustained evidence required)', () => {
    const veryBadDay = [makeEvidence(0, {
      sleepQuality: 10, physicalEnergy: 12, mentalClarity: 14,
      overwhelm: 95, emotionalResistance: 88, stressPressure: 85,
    }, 0.9)]
  ...
  it('produces a valid mode and trajectory on cold start', () => {
    ...
    expect(['EXPANDING', 'STABLE', 'FRAGILE', 'CONTRACTING']).toContain(result.state.currentTrajectory)
  })
})
...
describe('Invariants: valid across all replay scenarios', () => {
  ...
  it('all scalar fields are bounded 0–100', () => {
    const scalarFields = [
      ...
      'momentumIntegrity', 'resilienceCapacity', 'overwhelmLevel', 'fragmentationLevel',
      'recoveryCapacity', 'meaningfulEngagement', 'deepWorkContinuity', 'behavioralVolatility',
      'adaptationReadiness', 'expansionReadiness',
    ] as const

    for (const evidence of allScenarios) {
      const { state } = evaluate({ evidence, signalSnapshots: [emptySnapshot(evidence.length - 1)] })
