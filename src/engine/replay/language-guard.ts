...
const DIAGNOSTIC_PATTERN = /\b(burned out|depressed|have burnout|clinically|diagnosis|diagnose|medical|disorder|syndrome)\b/gi
...
export function guardText(text: string, section: ReplaySection): LanguageGuardResult {
  ...
  if (DIAGNOSTIC_PATTERN.test(result)) {
    ...
    if (import.meta.env.DEV) {
      console.error(`[ReplayLanguageGuard] Diagnostic label found and stripped in section "${section}": "${text}"`)
    }
  }

  for (const [pattern, replacement] of PREDICTION_REPLACEMENTS) {
    if (pattern.test(result)) {
      violations.push({ section, rule: 'prediction', correction: `replaced with "${replacement}"` })
      result = result.replace(pattern, replacement)
    }
  }
  ...
  if (FORMULA_PATTERN.test(result)) {
    ...
    if (import.meta.env.DEV) {
      console.error(`[ReplayLanguageGuard] Formula leak found and stripped in section "${section}": "${text}"`)
    }
  }

  return { text: result.trim(), violations, causalGuardFired, diagnosticGuardFired, formulaGuardFired }
}

export function guardAllText(
  texts: Array<{ text: string; section: ReplaySection }>,
): { results: LanguageGuardResult[]; violations: TrustViolation[]; causalGuardFired: boolean; diagnosticGuardFired: boolean; formulaGuardFired: boolean } {
