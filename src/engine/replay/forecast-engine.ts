...
const OUTCOME_OBSERVATIONS: Partial<Record<PeriodPair, string>> = {
  'RECOVERY->STABILIZING': 'Patterns have tended toward a stabilizing phase — scores in this window have often begun leveling off.',
  'RECOVERY->FOCUSED': 'In patterns where recovery was observed, focused execution phases followed on a number of occasions.',
  'RECOVERY->INSTABILITY': 'In patterns where recovery appeared, instability has sometimes persisted — a common observation after extended dips.',
  'STABILIZING->FOCUSED': 'Stabilizing periods have tended to precede focused execution in historical patterns.',
  'STABILIZING->EXPANDING': 'An expanding phase appeared alongside several stabilizing periods in the record.',
  'STABILIZING->INSTABILITY': 'Instability was observed following some stabilizing periods — often in patterns where consistency was lower.',
  'FOCUSED->EXPANDING': 'Expanding output phases appeared alongside several focused periods in the pattern history.',
  'FOCUSED->INSTABILITY': 'Instability was observed following focused periods on some occasions — common when overextension was present.',
  'FOCUSED->RECOVERY': 'A recovery phase appeared after focused periods in a number of cases — often preceded by high output density.',
  'EXPANDING->FOCUSED': 'Focused phases tended to follow expanding ones, often as a natural consolidation.',
  'EXPANDING->INSTABILITY': 'Instability appeared following expanding phases in some historical instances.',
  'INSTABILITY->STABILIZING': 'Stabilizing patterns appeared after instability phases in the record — a commonly observed recovery path.',
  'INSTABILITY->RECOVERY': 'Recovery activation was observed following several instability periods.',
  'INSTABILITY->FOCUSED': 'Direct transitions from instability to focused phases appeared on occasion — typically where strong re-engagement was present.',
}
...
export function buildForecast(
  ...
  if (eligiblePaths.length === 0) {
    return {
      forecast: {
        ...
        confidence: 'LOW',
        suppressed: true,
        suppressionReason: 'Not enough transition data from the current period to surface patterns.',
      },
      violations: [],
    }
  }
  ...
  const outcomes: ForecastOutcome[] = topPaths.map((path, i) => {
    const probability = path.frequency >= 0.5 ? 'LIKELY' : path.frequency >= 0.25 ? 'POSSIBLE' : 'UNLIKELY'
    // avgDurationBefore is used as a proxy for time-to-outcome (see design doc)
    const avg = path.avgDurationBefore
    const estimatedDaysRange = avg > 0
      ? { min: Math.max(1, Math.round(avg * 0.5)), max: Math.round(avg * 1.5) }
      : null
  ...
  const guarded = guardAllText(outcomes.map((o) => ({ text: o.observation, section: 'forecast' as const })))
