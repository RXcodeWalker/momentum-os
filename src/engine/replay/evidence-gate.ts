...
export function computeSuppressionMap(
  ...
  const narrative = (() => {
    if (totalCheckIns < 5) return { suppressed: true, reason: 'At least 5 check-ins needed to build a timeline.' }
    if (evidenceDays < config.minimumEvidenceDays) return { suppressed: true, reason: 'Not enough data days in this window.' }
    return { suppressed: false, reason: null }
  })()

  const attribution = (() => {
    if (totalCheckIns < 7) return { suppressed: true, reason: 'At least 7 check-ins needed for factor analysis.' }
    if (confidenceBand === 'LOW') return { suppressed: true, reason: 'Not enough consistent data to attribute patterns.' }
    return { suppressed: false, reason: null }
  })()

  const transition = (() => {
    if (totalCheckIns < 10) return { suppressed: true, reason: 'At least 10 check-ins needed to detect transitions.' }
    if (!priorSnapshot) return { suppressed: true, reason: 'No prior period data available for comparison.' }
    return { suppressed: false, reason: null }
  })()

  const forecast = (() => {
    if (totalCheckIns < 21) return { suppressed: true, reason: 'At least 21 check-ins needed for pattern-based forecasting.' }
    if (confidenceBand === 'LOW') return { suppressed: true, reason: 'Evidence confidence is too low to generate forecasts.' }
    return { suppressed: false, reason: null }
  })()
