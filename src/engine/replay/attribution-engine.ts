...
import type {
  AttributionFactor,
  AttributionFactorKind,
  AttributionDirection,
  ReplayAttribution,
} from "@/core/contracts/replay/attribution";
...
function buildSleepFactor(
  ...
  const snapshot =
    evidence.snapshots["W7"] ?? evidence.snapshots["W14"] ?? evidence.snapshots["W28"];
  ...
  const scoreDelta =
    sleepTrend?.currentWindowAvg != null && sleepTrend?.priorWindowAvg != null
...
function buildDistractionFactor(
  evidence: BehavioralEvidence,
  evidenceDays: number,
): AttributionFactor | null {
  const snapshot =
  ...
  return {
    ...
    scoreDelta:
      direction === "LIMITING" ? -avgDistractionCount * 2 : direction === "SUPPORTIVE" ? 3 : 0,
    confidence: evidenceDays >= 7 ? "MEDIUM" : "LOW",
    evidenceDays,
  };
}

function buildBlockerFactor(
  evidence: BehavioralEvidence,
  evidenceDays: number,
): AttributionFactor | null {
  const snapshot =
...
function buildFocusFactor(
  ...
  const scoreDelta =
    focusTrend.currentWindowAvg != null && focusTrend.priorWindowAvg != null
...
function buildConsistencyFactor(
  evidence: BehavioralEvidence,
  evidenceDays: number,
): AttributionFactor | null {
  const snapshot =
...
export function buildAttribution(
  ...
  const factors = rawFactors.filter(
    (f): f is AttributionFactor => f !== null && f.evidenceDays >= 3,
  );

  // Guard all observation text
  const guarded = guardAllText(
    factors.map((f) => ({ text: f.observation, section: "attribution" as const })),
  );
