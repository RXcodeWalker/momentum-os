...
function countEventTypeInWindow(events: BehavioralEvent[], type: BehavioralEventType, start: string): number {
...
function eventToEntry(
  ...
  switch (eventType) {
    ...
    case 'SCORE_THRESHOLD_CROSSED': {
      ...
      const significance: ReplayEntrySignificance = (p.threshold === 60 || p.threshold === 45) ? 'HIGH' : 'MEDIUM'
      return {
        entryId,
...
export function buildNarrative(
  ...
  for (const entry of candidates) {
    const dupe = deduped.find(
      (e) =>
        e.kind === entry.kind &&
        Math.abs(
          (new Date(e.sessionDate).getTime() - new Date(entry.sessionDate).getTime()) /
            86400000,
  ...
  const guarded = guardAllText(trimmed.map((e) => ({ text: e.headline, section: 'narrative' as const })))
