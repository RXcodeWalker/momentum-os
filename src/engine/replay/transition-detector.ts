...
import type {
  ...
  TransitionDirection,
} from "@/core/contracts/replay/transition";
...
export function buildTransitionSummary(
  ...
  const guarded = guardAllText(
    transitions.map((t) => ({ text: t.observation, section: "transition" as const })),
  );
  const guardedTransitions = transitions.map((t, i) => ({
    ...t,
    observation: guarded.results[i].text,
  }));
  ...
  const netDirection: TransitionDirection =
    upwardCount > downwardCount ? "UPWARD" : downwardCount > upwardCount ? "DOWNWARD" : "LATERAL";

  const w7 = evidence.snapshots["W7"];
  const prior = evidence.snapshots["W7_PRIOR"];
  const scoreDeltaTotal =
    w7 && prior ? w7.metrics.avgExecutionScore - prior.metrics.avgExecutionScore : null;
