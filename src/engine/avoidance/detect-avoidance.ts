import type { AvoidanceProfile, AvoidancePatternId, DetectedAvoidancePattern, AvoidanceEvidence } from '@/core/contracts/avoidance'
...
function detectMaintenanceLoop(
  ...
  const completedAdmin = completedTasks.filter((t) => t.type === 'admin' || t.type === 'shallow').length
  ...
  const negativeProgressRate = recentCheckIns.length > 0 ? negativeProgressCount / recentCheckIns.length : 0
  ...
  const deepRescheduledSignal = deepRescheduledTasks.length > 0
...
function detectPreparationEscape(
  ...
  const prepDominanceRatio = completedTasks.length > 0 ? completedPrep.length / completedTasks.length : 0
  ...
  const concurrentNegativeRate = recentCheckIns.length > 0
    ? noProgressWithCompletionCount / recentCheckIns.length
    : 0
  ...
  const deepBlockerRate = recentCheckIns.length > 0 ? deepBlockers.length / Math.max(1, recentCheckIns.length) : 0
  ...
  const noFollowThroughSignal = prepDayCount > 0 && completedTasks.filter((t) => t.type === 'deep').length === 0
...
function detectFragmentationEscape(
  ...
  const completedTasks = tasks.filter((t) => t.done)
  const avgEstMin = completedTasks.length > 0
    ? avg(completedTasks.map((t) => t.estMin))
    : 30
  const taskSizeSignal = (avgEstMin < 20 && completedTasks.length >= 5)
    ? clamp(1 - avgEstMin / 20, 0, 1)
    : 0
  ...
  const focusSignal = recentCheckIns.length > 0
    ? clamp(highCompletionLowFocusDays / recentCheckIns.length, 0, 1)
    : 0
  ...
  const distractionSignal = distractionTypeDensity >= 3
    ? clamp((distractionTypeDensity - 2) / 5, 0, 1)
    : 0

  const taskTypes = new Set(completedTasks.map((t) => t.type))
  const typeSwitchCount = taskTypes.size
  const taskTypeSwitchSignal = typeSwitchCount >= 3
    ? clamp((typeSwitchCount - 2) / 3, 0, 1)
    : 0
...
function detectAdvancementDeferral(
  ...
  const deepRescheduledSignal = deepTasksRescheduled.length > 0
    ? clamp(deepTasksRescheduled.length / Math.max(1, tasks.filter((t) => t.type === 'deep').length), 0, 1)
    : 0

  const recentHistory = history.filter((d) => d.date >= windowStart)
  const avgCompletionRatio = recentHistory.length > 0
  ...
  const completionAcceptableSignal = avgCompletionRatio >= 0.5
    ? clamp(avgCompletionRatio, 0, 1)
    : 0

  // Energy not depleted on low-progress days
  const lowProgressDays = last5.filter((c) => c.meaningfulProgress === 'no')
  const avgEnergyOnLowDays = lowProgressDays.length > 0
    ? avg(lowProgressDays.map((c) => c.energy))
    : 0
  const energyNotDepletedSignal = avgEnergyOnLowDays >= 5
    ? clamp((avgEnergyOnLowDays - 4) / 6, 0, 1)
    : 0
...
export function detectAvoidance(inputs: AvoidanceDetectionInputs): AvoidanceProfile | null {
  ...
  const maintenanceResult = detectMaintenanceLoop(
    inputs.tasks, inputs.checkIns, inputs.history, inputs.blockerHistory, windowStart,
  )
  const preparationResult = detectPreparationEscape(
    inputs.tasks, inputs.checkIns, inputs.blockerHistory, windowStart,
  )
  const fragmentationResult = detectFragmentationEscape(
    inputs.tasks, inputs.checkIns, inputs.distractionLog, windowStart,
  )
  const advancementResult = detectAdvancementDeferral(
    inputs.tasks, inputs.checkIns, inputs.history, windowStart,
  )
  ...
  const patterns: DetectedAvoidancePattern[] = (
    ...
    const finalSeverity: RiskLevel =
      confidence === 'LOW' && score > 0.7 ? 'LOW' : severity
  ...
  const minConfidence: ConfidenceBand = activeDetected.length > 0
