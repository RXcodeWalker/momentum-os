...
function computeCompleteness(checkIn: CheckIn): number {
  const present = BRIDGE_CONFIG.requiredFields.filter(
    f => (checkIn as Record<string, unknown>)[f] !== undefined &&
...
function checkInToDailyInputs(checkIn: CheckIn): DailyInputs {
  ...
  const pacingQuality      = clamp(
    (1 - fragmentation / 100) * 60 + physicalEnergy * 0.4,
  )
  ...
  return {
    ...
    behavioralInputs: { fragmentationLevel: fragmentation, distractionPatterns: fragmentation, avoidancePressure, pacingQuality },
  }
}
  ...
  return sorted.map(day => {
    ...
    return {
      ...
      inputs: {
        ...
        executionInputs: {
          meaningfulAdvancementQuality: day.planned > 0
            ? clamp((day.completed / day.planned) * 100)
            : 50,
          deepWorkContinuity: clamp(day.focus * 10),
          executionIntegrity: clamp(day.executionScore),
        },
        behavioralInputs: {
          fragmentationLevel:  clamp((day.distractions / BRIDGE_CONFIG.distractionTypesSaturate) * 100),
          distractionPatterns: clamp((day.distractions / BRIDGE_CONFIG.distractionTypesSaturate) * 100),
