...
function evaluateCandidates(
  ...
  return [
    ...
    },
    {
      ...
      evidence: [
        `emotionalFriction deviation: ${(deviations.emotionalFriction ?? 0).toFixed(1)}`,
      ],
    },
  ]
}
...
export function evaluatePatterns(
  ...
  const mp = (
    reflectionOutput as ReflectionOutput & { _mpRaw?: 'yes' | 'partial' | 'no' | null }
  )._mpRaw
  ...
  for (const candidate of sorted) {
    ...
    surfaced.push(obs(candidate.code, OBSERVATION_TEXTS[candidate.code], 'HIGH', candidate.evidence))
  }

  // Collect suppressed eligible candidates that weren't surfaced
  for (const candidate of sorted) {
    if (!surfaced.find((o) => o.code === candidate.code) && !suppressedCodes.includes(candidate.code)) {
      suppressedCodes.push(candidate.code)
    }
  }
