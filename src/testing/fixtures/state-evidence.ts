...
export function buildEvidence(
  ...
  return Array.from({ length: days }, (_, day) =>
    makeEvidence(
      day,
      typeof overrides === 'function' ? overrides(day) : overrides,
      completeness,
    ),
  )
}
...
export function buildScenario_SingleBadDayAfterStreak(): SessionEvidence[] {
  const strong = buildEvidence(10, {
    sleepQuality: 76, physicalEnergy: 74, overwhelm: 24,
    meaningfulAdvancementQuality: 75, executionIntegrity: 74,
    fragmentationLevel: 20, pacingQuality: 74,
  })
  const badDay = makeEvidence(10, {
    sleepQuality: 22, physicalEnergy: 25, mentalClarity: 28,
    overwhelm: 82, fragmentationLevel: 78,
  })
