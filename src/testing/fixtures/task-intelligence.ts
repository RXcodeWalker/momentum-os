...
export function makeTask(overrides: Partial<Task> & { title: string }): Task {
  ...
  return {
    ...
    stateCompatibility:
      overrides.stateCompatibility ?? BANDS_BY_CATEGORY[resolvedCategory],
  }
}
...
export function buildScenario_MaintenanceLooping(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      makeTask({ title: 'Inbox sweep', category: 'MAINTENANCE', meaningfulness: 35, repeatedDeferralCount: 0 }),
      makeTask({ title: 'Expense reports', category: 'MAINTENANCE', meaningfulness: 30, repeatedDeferralCount: 0 }),
      makeTask({ title: 'Weekly admin', category: 'MAINTENANCE', meaningfulness: 40, repeatedDeferralCount: 1 }),
      makeTask({ title: 'Ship feature', category: 'ADVANCEMENT', meaningfulness: 85, emotionalResistance: 55, repeatedDeferralCount: 3 }),
      makeTask({ title: 'Write proposal', category: 'ADVANCEMENT', meaningfulness: 80, emotionalResistance: 50, repeatedDeferralCount: 2 }),
    ],
  }
}
...
export function buildScenario_PreparationEscape(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      ...
      makeTask({ title: 'Launch MVP', category: 'ADVANCEMENT', meaningfulness: 90, repeatedDeferralCount: 3 }),
      makeTask({ title: 'Client deliverable', category: 'ADVANCEMENT', meaningfulness: 88, repeatedDeferralCount: 2 }),
    ],
  }
}
...
export function buildScenario_ResistanceClustering(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      makeTask({ title: 'Difficult conversation', category: 'ADVANCEMENT', meaningfulness: 80, emotionalResistance: 75, repeatedDeferralCount: 3 }),
      makeTask({ title: 'Complex refactor', category: 'ADVANCEMENT', meaningfulness: 78, emotionalResistance: 70, repeatedDeferralCount: 2 }),
      makeTask({ title: 'Quick email', category: 'MAINTENANCE', meaningfulness: 25, emotionalResistance: 10 }),
    ],
  }
}
...
export function buildScenario_RecoverySequencing(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      makeTask({ title: 'Rest walk', category: 'RECOVERY', recoveryCompatibility: 90, recoveryCost: 10, cognitiveLoad: 10 }),
      makeTask({ title: 'Light admin', category: 'MAINTENANCE', recoveryCompatibility: 75, recoveryCost: 15, cognitiveLoad: 20 }),
      makeTask({ title: 'Major launch', category: 'ADVANCEMENT', recoveryCost: 80, cognitiveLoad: 85, emotionalResistance: 60 }),
    ],
  }
}
...
export function buildScenario_FocusedDeepWork(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      makeTask({ title: 'Check slack', category: 'MAINTENANCE', meaningfulness: 20, deepWorkCompatibility: 15, fragmentationRisk: 70 }),
      makeTask({ title: 'Sort inbox', category: 'MAINTENANCE', meaningfulness: 25, deepWorkCompatibility: 10, fragmentationRisk: 65 }),
      makeTask({ title: 'Design system architecture', category: 'ADVANCEMENT', meaningfulness: 90, deepWorkCompatibility: 92, cognitiveLoad: 70, leverageWeight: 88 }),
      makeTask({ title: 'Quick standup prep', category: 'SUPPORT', meaningfulness: 30, deepWorkCompatibility: 20 }),
    ],
  }
}
...
export function buildScenario_ExpandingStretch(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      makeTask({ title: 'File expenses', category: 'MAINTENANCE', meaningfulness: 25, cognitiveLoad: 15 }),
      makeTask({ title: 'Learn new framework', category: 'GROWTH', meaningfulness: 75, cognitiveLoad: 65, leverageWeight: 80 }),
      makeTask({ title: 'Build v2 prototype', category: 'ADVANCEMENT', meaningfulness: 88, cognitiveLoad: 72, leverageWeight: 90 }),
    ],
  }
}
...
export function buildScenario_RecoveryMaintenanceExpected(): { state: UserState; tasks: Task[] } {
  return {
    ...
    tasks: [
      ...
      makeTask({ title: 'Tidy workspace', category: 'MAINTENANCE', meaningfulness: 30, repeatedDeferralCount: 0 }),
      makeTask({ title: 'Light email', category: 'MAINTENANCE', meaningfulness: 25, repeatedDeferralCount: 0 }),
      makeTask({ title: 'Deferred launch', category: 'ADVANCEMENT', meaningfulness: 85, repeatedDeferralCount: 3 }),
    ],
  }
}
