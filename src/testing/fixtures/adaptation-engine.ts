...
function makeDefaultEvaluation(directives: AdaptationDirective[] = []): InterventionEvaluationResult {
  return {
    interventions: directives.length > 0
      ? [{
...
export function makeAdaptationInput(overrides?: {
  ...
  const signalSnapshot = signals.length > 0
