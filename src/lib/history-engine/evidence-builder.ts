...
import type { EvidenceConfidence, EvidenceConfidenceFactors } from '@/core/contracts/history/confidence'
import type {
  BehavioralEvidence,
  EvidenceSummary,
  DerivedMetrics,
...
function computeEvidenceConfidence(
  ...
  const recentGapDays = lastEvidenceDate
    ? Math.floor(
        (Date.now() - new Date(lastEvidenceDate).getTime()) / (24 * 60 * 60 * 1000),
      )
    : Infinity
...
): Partial<Record<TrendMetricKey, TrendRecord>> {
