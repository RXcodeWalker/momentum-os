...
export function computeSnapshot(
  ...
  const windowHistory = history.filter((d) => d.date > windowStart && d.date <= windowEnd);
  ...
  const completionRates = windowHistory.map((d) => (d.planned > 0 ? d.completed / d.planned : 1));
  ...
  const windowDistrLog = distractionLog.filter((d) => d.date > windowStart && d.date <= windowEnd);
  ...
  const dominantDistractionType = distractionProfile.length > 0 ? distractionProfile[0].id : null;

  // Blocker frequency — O(n_blockerHistory) pass
  const windowBlockers = blockerHistory.filter((b) => b.date > windowStart && b.date <= windowEnd);
  ...
  const consistencyRate =
    evidenceDays > 0
      ? Math.round(
          (windowHistory.filter((d) => d.executionScore >= 70).length / evidenceDays) * 100,
        )
      : 0;
